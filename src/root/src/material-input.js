class MaterialInput extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.attachShadow({ mode: 'open' }).innerHTML = `
            ...
            <input tabindex="-1" 
              class="material-input__hidden-input" 
              style="pointer-events: none; margin:0; border: 0; height: 0; opacity: 0; position: absolute; top:0px; left: 0px;" name="name"/>
        `
    ...
    if (window['WebComponents'] !== undefined && window['WebComponents'].flags.shadow === true) { // eslint-disable-line no-undef
      window['WebComponents'].ShadowCSS.shimStyling(this.shadowRoot, 'material-input') // eslint-disable-line no-undef
    }
    // add hidden input
    ...
    this.$hiddenInput = this.shadowRoot.querySelector('.material-input__hidden-input')
    // elements
